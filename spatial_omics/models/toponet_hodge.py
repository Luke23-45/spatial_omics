...
class SimplicialHodgeLayer(nn.Module):
    def __init__(self, dim: int, polynomial_order: int, dropout: float, restriction_hidden_dim: int) -> None:
        ...
        self.node_endpoint_proj = nn.Linear(dim, dim, bias=False)
        self.edge_to_node_proj = nn.Linear(dim, dim, bias=False)
        self.triangle_to_edge_proj = nn.Linear(dim, dim, bias=False)
        self.edge_to_triangle_proj = nn.Linear(dim, dim, bias=False)

        self.node_restrict = nn.Sequential(
            nn.Linear(dim * 2, restriction_hidden_dim),
            nn.GELU(),
            nn.Linear(restriction_hidden_dim, dim),
        )
        self.edge_restrict = nn.Sequential(
            nn.Linear(dim * 2, restriction_hidden_dim),
            nn.GELU(),
            nn.Linear(restriction_hidden_dim, dim),
        )
        self.triangle_restrict = nn.Sequential(
            nn.Linear(dim * 2, restriction_hidden_dim),
            nn.GELU(),
            nn.Linear(restriction_hidden_dim, dim),
        )
        self.edge_triangle_restrict = nn.Sequential(
            nn.Linear(dim * 2, restriction_hidden_dim),
            nn.GELU(),
            nn.Linear(restriction_hidden_dim, dim),
        )
        ...
        self.component_gate = nn.Sequential(
            nn.Linear(dim * 4, restriction_hidden_dim),
            nn.GELU(),
            nn.Linear(restriction_hidden_dim, 3),
        )
        self.grad_transform = nn.Linear(dim, dim, bias=False)
        self.curl_transform = nn.Linear(dim, dim, bias=False)
        self.harm_transform = nn.Linear(dim, dim, bias=False)
    ...
    @staticmethod
    def _masked_average(mask: torch.Tensor, values: torch.Tensor) -> torch.Tensor:
        weights = mask.float()
    ...
    def _approx_harmonic_projection(
        self,
        edge_signal: torch.Tensor,
        grad_signal: torch.Tensor,
        curl_signal: torch.Tensor,
        edge_mask: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        weights = edge_mask.float().unsqueeze(-1)
        x = edge_signal * weights
        g = grad_signal * weights
        c = curl_signal * weights
        ...
        alpha = (xg * cc - xc * gc) / det
        beta = (xc * gg - xg * gc) / det
        grad_proj = alpha.unsqueeze(1) * g
        curl_proj = beta.unsqueeze(1) * c
        harmonic = (x - grad_proj - curl_proj) * weights
        return grad_proj, curl_proj, harmonic
    ...
    def forward(
        ...
        abs_b1 = b1.abs()
        abs_b2 = b2.abs()
        abs_b2_t = abs_b2.transpose(1, 2)

        src_nodes = torch.bmm(neg_b1, node_state)
        dst_nodes = torch.bmm(pos_b1, node_state)
        src_message = self._restricted_message(src_nodes, edge_state, self.node_restrict, self.node_endpoint_proj)
        dst_message = self._restricted_message(dst_nodes, edge_state, self.node_restrict, self.node_endpoint_proj)
        endpoint_scores = torch.cat(
            [
                self.endpoint_score(torch.cat([src_nodes, edge_state], dim=-1)),
                self.endpoint_score(torch.cat([dst_nodes, edge_state], dim=-1)),
            ],
            dim=-1,
        )
        endpoint_weights = torch.softmax(endpoint_scores, dim=-1)
        grad_message = endpoint_weights[..., :1] * src_message + endpoint_weights[..., 1:] * dst_message
        ...
        component_logits = self.component_gate(torch.cat([edge_state, grad_component, curl_component, harmonic_component], dim=-1))
        component_weights = torch.softmax(component_logits, dim=-1)
        edge_fused = (
            component_weights[..., :1] * self.grad_transform(grad_component)
            + component_weights[..., 1:2] * self.curl_transform(curl_component)
            + component_weights[..., 2:] * self.harm_transform(harmonic_component)
        )

        node_from_edges = self._masked_average(abs_b1, edge_state)
        node_residual = self._restricted_message(node_from_edges, node_state, self.edge_restrict, self.edge_to_node_proj)
        node_update = self.node_self(self._poly_apply(l0, node_state, self.theta0)) + node_residual
        ...
        edge_update = edge_diffusion + edge_fused
...
class TopoNetHodgeClassifier(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.dropout = nn.Dropout(dropout)
        self.topology_context = nn.Sequential(
            nn.Linear(6, model_dim),
            nn.GELU(),
            nn.LayerNorm(model_dim),
        )
        self.readout = nn.Sequential(
            nn.Linear(model_dim * 7, model_dim * 2),
            nn.GELU(),
            nn.Dropout(dropout),
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        for layer in self.layers:
            node_state, edge_state, triangle_state, harmonic_state = layer(
                node_state,
                edge_state,
                triangle_state,
                batch["B1"],
        ...
        node_count = node_mask.sum(dim=1, keepdim=True).float()
        edge_count = edge_mask.sum(dim=1, keepdim=True).float()
        tri_count = triangle_mask.sum(dim=1, keepdim=True).float()
        euler = node_count - edge_count + tri_count
        harmonic_energy = (harmonic_state.pow(2) * edge_mask.unsqueeze(-1).float()).sum(dim=(1, 2)).unsqueeze(-1)
        edge_energy = (edge_state.pow(2) * edge_mask.unsqueeze(-1).float()).sum(dim=(1, 2)).unsqueeze(-1).clamp_min(1e-6)
        harmonic_ratio = harmonic_energy / edge_energy
        betti_proxy = harmonic_state.norm(dim=-1)
        betti_proxy = (betti_proxy * edge_mask.float()).sum(dim=1, keepdim=True) / edge_mask.float().sum(dim=1, keepdim=True).clamp_min(1.0)
        topo_context = torch.cat([counts, euler, harmonic_ratio, betti_proxy], dim=-1)
        topo_context = self.topology_context(topo_context)
        pooled = torch.cat([node_mean, node_max, edge_mean, edge_max, tri_mean, tri_max, topo_context], dim=-1)
