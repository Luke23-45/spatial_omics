...
from spartial_omics.models.gnn_baselines import GNNBaselineConfig, run_gnn_baselines_study, save_gnn_baseline_results


def main() -> int:
    parser = argparse.ArgumentParser(description="Run grouped-CV standard GNN baselines on a prepared spatial-omics study.")
    ...
    parser.add_argument("--model-names", nargs="+", default=None, choices=["gcn", "graphsage", "gat", "gin"])
    ...
    cfg = GNNBaselineConfig(
        ...
        model_names=tuple(args.model_names) if args.model_names else ("gcn", "graphsage", "gat", "gin"),
