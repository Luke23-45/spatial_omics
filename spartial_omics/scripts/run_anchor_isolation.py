...
from spartial_omics.evaluation.anchor_isolation import AnchorIsolationConfig, run_anchor_isolation, save_anchor_isolation_results


def main() -> int:
    parser = argparse.ArgumentParser(description="Run the isolated anchor-router study for Spectra.")
    ...
    parser.add_argument("--topk-cells", type=int, default=8)
    ...
    cfg = AnchorIsolationConfig(
        ...
        topk_cells=args.topk_cells,
        random_state=args.random_state,
    )
    results = run_anchor_isolation(cfg)
    save_anchor_isolation_results(results, cfg.output_dir)
    return 0
